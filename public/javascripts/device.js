// Generated by CoffeeScript 1.7.1
(function() {
  $(function() {
    var allhide, connecting, dc, imageViewing, initializing, peerIDsetting, showError, showImage, showMessage, waiting;
    dc = new ns.DeviceClass();
    peerIDsetting = function(id) {
      return $('#device-id').text(id);
    };
    allhide = function() {
      $('#initialize').hide();
      $('#waiting').hide();
      $('#connecting').hide();
      $('#image-viewing').hide();
      return $('#video-container').hide();
    };
    initializing = function() {
      allhide();
      return $('#initialize').show();
    };
    waiting = function() {
      allhide();
      $('#message').text("");
      $('#waiting').show();
      return $('#terminate').focus();
    };
    connecting = function() {
      allhide();
      $('#connecting').show();
      return $('#end-call').focus();
    };
    imageViewing = function() {
      allhide();
      $('#image-viewing').show();
      return $('#close-image').focus();
    };
    showError = function(errMessage) {
      console.log("showError :" + errMessage);
      return alert(errMessage);
    };
    showMessage = function(message) {
      return $('#message').text(message);
    };
    showImage = function(image) {
      $('#sent-image')[0].src = image;
      return imageViewing();
    };
    $('#end-call').click(function() {
      dc.closeCall();
      return waiting();
    });
    $('#close-image').click(function() {
      $('#sent-image')[0].src = "";
      return connecting();
    });
    $('#terminate').click(function() {
      dc.terminate();
      return window.open('about:blank', '_self').close();
    });
    dc.onOpen(peerIDsetting);
    dc.onError(showError, waiting);
    dc.onConnection(showMessage, showImage);
    dc.onCall($('#monitor-video'), connecting, waiting);
    return dc.initialize($('#device-video'), initializing, waiting);
  });

}).call(this);
